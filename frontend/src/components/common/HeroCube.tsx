...
const keyLight = new THREE.DirectionalLight(0xfff5e6, 2.0) // Warm white
...
// Ambient light
const ambient = new THREE.AmbientLight(0x403830, 0.7) // Warm ambient
scene.add(ambient)

// Point lights for shine
...
/* 🥂 CHAMPAGNE GOLD - Soft, Premium, Eyes-friendly */
const cubeColor = 0xf7e7ce      // Soft champagne
const emissiveColor = 0x332211   // Dark warm brown

// Create material once for all cubes
const material = new THREE.MeshStandardMaterial({
  color: cubeColor,
  emissive: emissiveColor,
  emissiveIntensity: 0.12,      // Subtle glow
  metalness: 0.82,               // Metallic but soft
  roughness: 0.18,               // Smooth
})
...
for(let x=-1; x<=1; x++){
  for(let y=-1; y<=1; y++){
    for(let z=-1; z<=1; z++){
      ...
      // Slight random rotation for each cube
...
for(let i = 0; i < particlesCount; i++) {
  ...
  // Champagne colors for particles
